...
//    private static final int MAX_SINGLE_PAGE_FLIP_ANIM_DURATION = 300;// in ms
    private static final int PEAK_ANIM_DURATION = 800;// in ms
    private static final int MAX_SINGLE_PAGE_FLIP_ANIM_DURATION = 300;// in ms
    ...
    private void recycleActiveViews() {
        ...
        if (mPreviousPage.valid) {
            ...
            mRecycler.addScrapView(mPreviousPage.view, mPreviousPage.position,
                    mPreviousPage.viewType);
            mPreviousPage.valid = false;
        }
        if (mCurrentPage.valid) {
            removeView(mCurrentPage.view);
            mRecycler.addScrapView(mCurrentPage.view, mCurrentPage.position,
                    mCurrentPage.viewType);
            mCurrentPage.valid = false;
        }
        if (mNextPage.valid) {
            removeView(mNextPage.view);
            mRecycler.addScrapView(mNextPage.view, mNextPage.position,
                    mNextPage.viewType);
